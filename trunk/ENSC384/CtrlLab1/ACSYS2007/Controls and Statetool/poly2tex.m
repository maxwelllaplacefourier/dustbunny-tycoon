function texTF = poly2tex_2(num,den,varargin)
    ...
    while den(1) == 0       %gets rid of leading 0s
        den = den(2:end);
    end
    while num(1) == 0
        num = num(2:end);
    ...
    for ii = 1:length(num)-1
        ...
        elseif num(ii) ~= 0
            texTF = [texTF sprintf('%4.4gs^{%d}+',abs(num(ii)),length(num)-ii)];
        end
        if num(ii+1) < 0
            texTF(end) = '-';
        end
    ...
    if length(den) ~= 1
        if abs(den(1)) == 1
            if den(1) > 0
                texTF = [texTF sprintf('%4.4g}{s^{%d}+',abs(num(length(num))),length(den)-1)];
            else
                texTF = [texTF sprintf('%4.4g}{-s^{%d}+',abs(num(length(num))),length(den)-1)];
            end
        else 
            texTF = [texTF sprintf('%4.4g}{%4.4gs^{%d}+',abs(num(length(num))),den(1),length(den)-1)];
        end
        if den(2) < 0
            texTF(end) = '-';
        end
        for ii = 2:length(den)-1
            ...
            elseif den(ii) ~= 0
                texTF = [texTF sprintf('%4.4gs^{%d}+',abs(den(ii)),length(den)-ii)];
            end
            if den(ii+1) < 0
                texTF(end) = '-';
            end
        end
            texTF = [texTF sprintf('%4.4g}$',abs(den(length(den))))];
    else
        texTF = [texTF sprintf('%4.4g}{%4.4g}$',num(length(num)),den(1))];
    end
