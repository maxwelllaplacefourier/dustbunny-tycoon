...
extern void *motor_pos_malloc(SimStruct *S);
#endif
...
/* Start of Functions in model "motor_pos_sf" */
/* Start the model */
#define MDL_START
...
# endif
  ...
  {
    /* simstruct variables */
    motor_pos_BlockIO *motor_pos_B = (motor_pos_BlockIO *) ssGetLocalBlockIO(S);
    motor_pos_Parameters *motor_pos_P = (motor_pos_Parameters *)
      ssGetLocalDefaultParam(S);

    /* Integrator Block: <S1>/Integrator */
    (void)memcpy(&motor_pos_B->Integrator, &motor_pos_P->Integrator_IC, 1 *
     sizeof(real_T));
...
{
  /* simstruct variables */
  motor_pos_ContinuousStates *motor_pos_X = (motor_pos_ContinuousStates*)
    ssGetContStates(S);

  /* Integrator Block: <S1>/Integrator */
  motor_pos_X->Integrator_CSTATE = 0.0;

  /* TransferFcn Block: <S1>/Electrical */
  motor_pos_X->Electrical_CSTATE = 0.0;

  /* TransferFcn Block: <S1>/Mechanical */
  motor_pos_X->Mechanical_CSTATE = 0.0;
}
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  motor_pos_B->Integrator = motor_pos_X->Integrator_CSTATE;
  ...
  /* Backlash: '<S1>/Backlash' */
  {
    ...
    if (ssIsMajorTimeStep(S)) {
      ...
      if ( motor_pos_B->Integrator <= lb ) {
        newMode = MODE_NEG_ENGAGED;
      } else if ( motor_pos_B->Integrator < ub ) {
    ...
    switch (((int_T*) ssGetDWork(S, 1))[0]) {
     case MODE_NEG_ENGAGED:
      motor_pos_B->Backlash = motor_pos_B->Integrator + halfDeadBand;
      break;
     case MODE_DISENGAGED:
      motor_pos_B->Backlash = *lastY;
      break;
  ...
  ((real_T *)ssGetOutputPortSignal(S,0))[0] = motor_pos_B->Backlash;
  ...
  /* TransferFcn Block: <S1>/Electrical */
  motor_pos_B->Electrical =
    1.7857142857142858E+003*motor_pos_X->Electrical_CSTATE;
   ...
   */
  if (ssIsMajorTimeStep(S)) {
    if (motor_pos_B->Electrical >= 4.0) {
      ((int_T*) ssGetDWork(S, 3))[0] = 1;
    } else if (motor_pos_B->Electrical <= (-4.0)) {
      ((int_T*) ssGetDWork(S, 3))[0] = -1;
    } else {
  ...
  if (((int_T*) ssGetDWork(S, 3))[0] == 1) {
    ...
    rtb_Current_Saturation = motor_pos_B->Electrical;
  }

  /* TransferFcn Block: <S1>/Mechanical */
  motor_pos_B->Mechanical =
    1.2626262626262628E+003*motor_pos_X->Mechanical_CSTATE;
  /* Gain: '<S1>/Motor Gain'
   *
   * Regarding '<S1>/Motor Gain':
   *   Gain value: 0.1
   */
  motor_pos_B->Motor_Gain = rtb_Current_Saturation * 0.1;
   ...
   *   Gain value: 0.1
   */
  motor_pos_B->Sum3 = (2.0 * rtb_Input_Voltage_Saturation)
    - (0.1 * motor_pos_B->Mechanical);
}
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
  ...
  /* Backlash Block: <S1>/Backlash */
  {
    ...
    if (*timeStampA < *timeStampB) {
      *timeStampA = t;
      ((real_T*) ssGetDWork(S,0))[1] = motor_pos_B->Backlash;
    } else {
      *timeStampB = t;
      ((real_T*) ssGetDWork(S,0))[3] = motor_pos_B->Backlash;
    }
  }
}
...
static void mdlDerivatives(SimStruct *S)
{
  ...
  /* TransferFcn Block: <S1>/Electrical */
  {
    ...
    dx[0] = motor_pos_B->Sum3;
    dx[0] += (-2.4107142857142862E+003)*motor_pos_X->Electrical_CSTATE;
  }
  ...
  /* TransferFcn Block: <S1>/Mechanical */
  {
    ...
    dx[0] = motor_pos_B->Motor_Gain;
    dx[0] += (-2.3989898989898992E+000)*motor_pos_X->Mechanical_CSTATE;
  }
}
...
static void mdlZeroCrossings(SimStruct *S)
{
  /* simstruct variables */
  motor_pos_BlockIO *motor_pos_B = (motor_pos_BlockIO *) ssGetLocalBlockIO(S);
  motor_pos_NonsampledZCs *motor_pos_NonsampledZC = (motor_pos_NonsampledZCs *)
  ...
  /* Backlash Block: <S1>/Backlash */
  {
    ...
    motor_pos_NonsampledZC->Backlash_NSZC[0] = motor_pos_B->Integrator -
      lastY[0] - halfDeadBand;
    motor_pos_NonsampledZC->Backlash_NSZC[1] = motor_pos_B->Integrator -
      lastY[0] + halfDeadBand;
  }

  /* Saturate Block: <S1>/Input Voltage Saturation */
  /* Do zero crossings for the upper limit */
  motor_pos_NonsampledZC->Input_Voltage_Saturation_NSZC[0] = *(((const
    real_T**)ssGetInputPortSignalPtrs(S, 0))[0]) -
    10.0;
  /* Do zero crossings for the lower limit */
  motor_pos_NonsampledZC->Input_Voltage_Saturation_NSZC[1] = *(((const
    real_T**)ssGetInputPortSignalPtrs(S, 0))[0]) -
    (-10.0);

  /* Saturate Block: <S1>/Current Saturation */
  /* Do zero crossings for the upper limit */
  motor_pos_NonsampledZC->Current_Saturation_NSZC[0] = motor_pos_B->Electrical -

    4.0;
  /* Do zero crossings for the lower limit */
  motor_pos_NonsampledZC->Current_Saturation_NSZC[1] = motor_pos_B->Electrical -
...
/* End of Functions in model "motor_pos_sf" */

#include "motor_pos_sf_reg.h"
