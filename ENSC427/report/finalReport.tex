\documentclass{enscStyle}
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage{multirow}
\usepackage{tabularx}
\usepackage{tikz}
\usepackage{pgf}
\usepackage{amsmath,amssymb}
\usepackage[utf8]{inputenc}

\title{Message Ferrying in Partitioned Ad Hoc Networks}

\coursecode{427}

\author{
    Group 9\\
    Dan Hendry, 301133878\\
    Timbo Yuen, 301023080\\
    Yazan Shehadeh,  301028275\\
}

\prof{Professor Ljiljana Trajkovic}
\date{March 14, 2010}

\begin{document}

\pagenumbering{roman}

\maketitle

\cleardoublepage

\setcounter{page}{2} % the table of contents will be shown as page iii

\input{chapters/abstract}

\cleardoublepage

\addcontentsline{toc}{chapter}{\hspace{13pt} Contents}
\tableofcontents

\listoftables
\addcontentsline{toc}{chapter}{\hspace{13pt} List of Tables}

\listoffigures
\addcontentsline{toc}{chapter}{\hspace{13pt} List of Figures}

\cleardoublepage
\pagenumbering{arabic}

%TEMP - to remove
%\input{chapters/latexExamples} %asdfasd

%adfgsa

% Main section

%Citation test (remove) ~\cite{wearable}

\input{chapters/introduction}

\input{chapters/simulationDesign}

\input{chapters/simulationResults}

\input{chapters/conclusion}

%Add chapters as needed

\addcontentsline{toc}{chapter}{\hspace{13pt} References}

\bibliography{finalReport}

%\appendix
%\renewcommand{\thechapter}{\Alph{chapter}}
%\input{chapters/appendix}

\end{document}
